---
description: Close the current work session — route each fact to where state lives, then write the residue the next session cannot re-derive, ending on one exact next action.
argument-hint: "[what you want carried over, in your own words]"
---

Run the `session-handoff` skill for the work in flight.

Follow it in order, without shortcuts:

1. **Route first.** For every fact you are tempted to record, decide where it belongs —
   tracker, diff, doctrine, memory, ADR, or this handoff. Write it in its authoritative home
...
Show the handoff before writing anything shared, and wait for an explicit yes. Do not move
