...
Start by understanding the project context. Then ask questions one at a time to refine the idea. Once you understand what is being built, present the design in sections, get approval, write the spec to `docs/specs/`, and transition to `voidcorp:writing-plans`. **No implementation skill, no code, no scaffolding until the spec is written and approved.**
...
### Step 10 — Transition to `voidcorp:writing-plans`
...
- **Downstream — `voidcorp:writing-plans`**: the ONLY post-brainstorming transition.
