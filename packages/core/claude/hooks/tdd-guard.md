...
PreToolUse hook on `Edit` / `Write`. Materializes the `voidcorp:tdd` skill's Iron Law mechanically: blocks edits to production paths that add behavior without a corresponding test change in the same staged change set, unless a legitimate bypass applies.
...
Read from `.voidcorp/config.json` of the consumer project:
...
If `.voidcorp/config.json` is absent, sensible defaults are used (see hook source).
...
- Per-repo: set `.voidcorp/config.json` `modes.tdd` to `strict` / `souple` / `exploratory`.
...
- `VOIDCORP_CONFIG` — optional, defaults to `.voidcorp/config.json`
...
- Per-session env override (`VOIDCORP_MODE_TDD`) — wire it up in the hook or rely on `.voidcorp/config.json` only? Lean wire (faster iteration during work).
