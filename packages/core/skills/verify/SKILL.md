...
| 12 | Spec / plan linked if work derives from one; plan resume point updated | `brainstorm` + `plan` |
...
For multi-step plans (composes with `plan`): update the plan's "Resume point" section with the completed step.
...
- **With `plan`**: item 12 — completion updates the plan resume point.
- **With `frontend-design` + `accessibility`**: item 6.
