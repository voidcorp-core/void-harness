...
## Shared artifacts - resolved at integration, not raced by workers

Some files are edited by almost every ticket, so partitioning alone does not prevent conflicts. Generated bytes are owned by reconciliation; append-only records are redesigned so each worker owns a distinct file.

- **Generated artifacts** - `packages/harness-graph/model.json`, `packages/core/graph/void-graph.mjs`, the `core-assets` mirror. Workers **MUST NOT commit** them. Reconciliation rebuilds them once after merging every branch, then the full suite gates the result.
- **ADRs** - each worker creates one collision-free UUID file through `void-harness decisions new`. There is no sequential number, reservation, shared index or reconciliation rewrite. `void-harness decisions check` gates the merged records and `decisions render` provides read-only projections.
- **Shared registries** - coverage-matrix, decision-matrix, routing `CLAUDE.md`/`AGENTS.md`. Each worker edits distinct rows; on a genuine same-row conflict reconciliation re-derives the row keeping both intents. `sync-agent-docs` gates parity at the end.

The rule: a worker owns subject files and isolated records; reconciliation alone owns generated artifacts.
...
- MUST NOT let a worker commit generated artifacts (`model.json`, `void-graph.mjs`, the `core-assets` mirror) or write an ADR shared index. Reconciliation rebuilds artifacts once; workers create isolated UUID decision files.
