...
- **`docs/DECISIONS.md`** (append-only log) — each worker appends **only its own** block. The reconciliation subagent **concatenates** the blocks in topological order; it never 3-way-merges the tail (append-vs-append is a false conflict). A per-decision-file layout (one file + a generated index, like ADRs) is the durable end state; concatenation is the interim that needs no migration.
...
The rule: a worker owns its **subject** files; the **shared-append** class is owned by integration. That is why the reconciliation subagent — not the worker — runs the artifact rebuild and the DECISIONS concatenation.
