...
`void-brainstorm` pressure-tests the idea and `void-plan` authors the plan. This skill sits between the written plan and implementation: it **critiques an already-written plan** from up to four expert lenses, surfaces findings one decision at a time, and appends an Implementation Tasks list to the plan. It is read-mostly — it **proposes**; the plan's author (you, via `void-plan`) **disposes**. It never writes code and never restructures the plan itself.
...
- **Alternatives are mandatory**: 2-3 approaches, weighting *minimal-viable* and *ideal-architecture* equally, each with effort/risk/reuse; recommend one; do not proceed without approval. (Authoring the chosen approach is `void-plan`' job — the lens forces the comparison, not the write-up.)
...
- **AI-slop risk**: classify the surface (marketing vs app-UI vs hybrid) and hold it to the matching bar; reject generic-AI-generated patterns. (Bar and blacklist re-derived from OpenAI's "designing delightful frontends" + Krug — cited in `.source`, composes with `void-frontend-design` / `void-ui-review`.)
...
**Output**: a DX scorecard (each dimension Score/Prior/Trend + TTHW tier + competitive rank); TTHW > 10 min is blocking; any dimension < 6 is critical DX debt. This lens judges the *plan's promises*; once the surface ships, `void-devex-audit` measures the reality against them.
...
- **Downstream of `void-plan`** (which authors the plan) and **upstream of implementation** (`void-implement`). It reviews the artifact `void-plan` produced.
- **Not `void-brainstorm`**: brainstorm pressure-tests the *idea* (is there demand?); plan-review critiques the *written plan* (is it the right shape, buildable, complete?). Different artifact, different question.
- **Not `void-code-review`**: that reviews a *diff*; this reviews a *plan* before any code exists.
...
- **Composes with `void-frontend-design` / `void-ui-review`** (the Design lens defers UI build-craft and audit to them), `void-devex-audit` (the DevEx lens's shipped-surface counterpart — this judges the plan, that measures the deployed reality), and `void-security-guidance` / `void-security-audit` (the Eng lens routes a deep security concern there).
...
- MUST NOT own plan structure or registries (Not-in-scope, diagrams) — that is `void-plan`.
- MUST NOT re-litigate the idea's demand — that is `void-brainstorm`.
- MUST NOT review code or a diff — that is `void-code-review`.
