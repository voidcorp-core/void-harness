...
Refactoring changes structure, not behavior. The moment behavior changes, you stop refactor and switch to `tdd`. Tidyings and Behavior Changes never share a commit. Named refactors from the Fowler catalog, executed mechanically. Tests stay green at every step.
...
"Let's rewrite this module" is not a refactor — it is a feature project. Goes through `brainstorm` + `plan` + `tdd`. Rewrites lose context, regress invariants, and never quite catch up to the original.
