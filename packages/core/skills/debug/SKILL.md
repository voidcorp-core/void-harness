...
A bug fix without a root cause is a band-aid. The bug recurs in three months under a different name, and the next debugger inherits the band-aid. `debug` enforces four phases (investigate → analyze → hypothesize → implement), with a failing test that reproduces the bug landing BEFORE the fix.
...
- The fix is a refactor (composes with `refactor` and `hexagonal-architecture` / `domain-driven-design`).
- Invoke the `doctrine-critic` agent to judge the structural root before the refactor.
- Compose with `brainstorm` + `plan` if the refactor is large enough to need a spec.
...
- **With `refactor`**: structural fixes compose with refactor's Two-Hat principle.
...
- MUST NOT defer to `migrations` for non-migration bugs.
