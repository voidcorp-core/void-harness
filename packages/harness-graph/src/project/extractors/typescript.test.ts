...
import { FIXTURE_COMPILERS, loadFixtureCompiler } from '../test-support.js';
import {
...
// Every suite runs once per compiler API a consumer can resolve, because the
// extractors, the resolver and the tsconfig reader call whichever API the
// analysed project installs, and 5.x and 6.x do not share every default.
const COMPILERS = await Promise.all(
	FIXTURE_COMPILERS.map(async ({ label, lookup }) => ({
		label,
		api: await loadFixtureCompiler(lookup),
	})),
);

describe.each(COMPILERS)('$label', ({ api: ts }) => {
	...
	describe('TypeScript config inheritance', () => {
		...
		it('matches official TypeScript option origins across an extends chain', () => {
			...
			const resolver = createTypeScriptModuleResolver(
				ts,
				new Set(['shared/value.ts', 'src/value.ts']),
			);
