...
it('connects implementation, tests and decisions with declared source hashes without rewriting sources', async () => {
 ...
 expect(result.graph.edges).toEqual(expect.arrayContaining([
  ...
  expect.objectContaining({ kind: 'constrained_by', from: projectFileId('src/service.ts'), to: constraint?.id }),
...
it('retains isolated and superseded decisions and does not arbitrate conflicting declarations', async () => {
 const f = await fixture({ [decisionPath]: adr(), 'docs/decisions-log/two.md': adr('adr:two', "affects: [src/service.ts]"), 'docs/decisions-log/three.md': adr('adr:three', "affects: [src/service.ts]").replace('supersedes: []', "supersedes: ['adr:two']"), 'src/service.ts': '' });
 const r = await f.build();
 expect(r.graph.nodes.filter(n => n.kind === 'decision')).toHaveLength(3);
 expect(r.graph.nodes.find(n => n.data['declarationId'] === 'adr:three')?.data['supersedes']).toEqual(['adr:two']);
 expect(r.graph.edges.filter(e => e.from === projectFileId('src/service.ts') && e.kind === 'decided_by')).toHaveLength(2);
});
it('reports missing paths and invalid references without inventing files or aborting unrelated declarations', async () => {
 const f = await fixture({ [decisionPath]: adr('adr:one', 'affects: [gone.ts, ../outside.ts]'), [invariantPath]: invariant() });
 const r = await f.build();
 expect(r.graph.nodes.filter(n => ['decision', 'invariant'].includes(n.kind))).toHaveLength(2);
 expect(r.issues).toEqual(expect.arrayContaining([expect.objectContaining({ code: 'knowledge-reference', path: decisionPath, message: expect.stringContaining('gone.ts') }), expect.objectContaining({ code: 'knowledge-reference', path: decisionPath, message: expect.stringContaining('../outside.ts') })]));
...
it('rejects every duplicate identity instead of choosing the first declaration', async () => {
 const f = await fixture({ [decisionPath]: adr(), [invariantPath]: invariant(), '.void/knowledge/invariants/duplicate.yaml': invariant() });
 const r = await f.build();
 expect(r.graph.nodes.filter(n => n.kind === 'invariant')).toHaveLength(0);
 expect(r.issues.filter(i => i.code === 'knowledge-duplicate')).toHaveLength(2);
});
it.each([
 ['missing frontmatter', 'Human prose'],
 ['duplicate key', adr().replace('title: Keep isolation', 'title: First\ntitle: Second')],
 ['alias', adr().replace('title: Keep isolation', 'title: &a Keep isolation\nextra: *a')],
 ['tag', adr().replace('title: Keep isolation', 'title: !execute payload')],
 ['deep nesting', adr('adr:one', `extra: ${'['.repeat(66)}x${']'.repeat(66)}`)],
 ...
 expect(r.issues).toContainEqual(expect.objectContaining({ code: 'knowledge-invalid', path: decisionPath }));
});
it('observes edited and deleted declarations across cache reuse and excludes other hidden knowledge', async () => {
 const f = await fixture({ [decisionPath]: adr(), [invariantPath]: invariant(), '.void/knowledge/intent.yaml': 'id: excluded', '.void/knowledge/invariants/nested/hidden.yaml': invariant() });
