#!/usr/bin/env bash
# no-ai-design-slop — PreToolUse hook. Reads the runtime JSON from stdin.
...
while IFS= read -r -d "$_HOOKLIB_RS" REC; do
  ...
  [[ "$FILE" =~ \.(tsx|jsx|css|scss)$ ]] || continue
  [[ "$FILE" =~ \.(test|spec)\.(tsx|jsx)$|/__generated__/|/__fixtures__/ ]] && continue
