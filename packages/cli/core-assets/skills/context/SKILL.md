...
- The plan and its resume point (composes with `plan` — the plan file is the source of truth, not the chat log).
...
Decompose long tasks into sub-tasks with their own gates (composes with `plan`). Each sub-task is small enough to run in a clean-ish window; the plan file stitches them across resets.
...
- **`debug`**: investigate without drowning the window — gather evidence in a subagent, bring back the timeline and root cause, not every log line read.
- **`plan`**: the plan file IS the on-disk state that survives a reset. Resume point is updated there, never only in chat.
