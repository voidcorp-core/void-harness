...
## Shared-append files — resolved at integration, not raced by workers

Some files are edited by *almost every* ticket, so the parallel-vs-sequential partition does not prevent their conflicts: even sequential branches cut from the same base collide on the same appended tail or the same regenerated bytes. This class is handled by protocol at **reconciliation**, never by workers racing on it. (Direct evidence: a batch of skill tickets each conflicted here — DECISIONS.md tails, ADR numbers, the 1.9 MB graph bundle.)

- **Generated artifacts** — `packages/harness-graph/model.json`, `packages/core/graph/void-graph.mjs`, the `core-assets` mirror. Workers **MUST NOT commit** them; a worker that changes a skill leaves the artifacts stale. The reconciliation subagent rebuilds them **once** after merging every branch (`graph build` + `build:void-graph` + `copy-core-assets`), and the full suite (`graph:check` + `graph:check-bundle`) gates the result. This removes the single largest conflict surface deterministically — N branches each regenerating the bundle become one rebuild.
- **ADR numbers** — `decisions/NNNN.md`. Two workers both taking "the next integer" collide on the filename and every cross-reference. The **launcher reserves a distinct ADR number per ticket at plan time** (from the current max on the base), handed to each worker; a worker that writes no ADR just doesn't use its reservation. Fallback: the reconciliation subagent renumbers deterministically in topological order before opening the PR.
- **`docs/DECISIONS.md`** (generated index) — each worker writes its decision as its **own new file** `docs/decisions-log/<YYYY-MM-DD>-<slug>.md`; distinct filenames never collide, so there is no shared append to race. The reconciliation subagent rebuilds the index once (`pnpm decisions:build`), like the other generated artifacts, and `pnpm decisions:check` gates drift. (The former append-then-concatenate interim is retired now the per-decision-file layout ships.)
- **Shared registries** — coverage-matrix, decision-matrix, routing `CLAUDE.md`/`AGENTS.md`. Each worker edits **distinct rows**; on a genuine same-row conflict the reconciliation subagent re-derives the row keeping both intents. `sync-agent-docs` parity gates CLAUDE.md/AGENTS.md at the end.

The rule: a worker owns its **subject** files; the **shared-append** class is owned by integration. That is why the reconciliation subagent — not the worker — runs the artifact rebuilds (graph bundle, `core-assets` mirror, DECISIONS index).
...
- MUST NOT let a worker commit generated artifacts (`model.json`, `void-graph.mjs`, the `core-assets` mirror) or grab an ad-hoc ADR number — the reconciliation subagent rebuilds artifacts once, ADR numbers are reserved at plan time.
