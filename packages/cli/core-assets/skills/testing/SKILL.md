...
`tdd` provides the cycle (when to write the test). This skill provides the technique (how to write a good one). Mocking is the last resort. Behavior, not implementation. Factories, not `beforeEach`. Pristine output is a passing condition.
...
| `Math.random` / UUID | seeded RNG passed via parameter (composes with `hexagonal-architecture` port pattern) |
...
- The ORM at the unit level (use pglite — see `tdd` anti-pattern)
...
- **With `tdd`**: `tdd` is the cycle (when), `testing` is the technique (how). RED step delegates the *how* here.
- **With `mutation-testing`**: high survivor rate is a `testing` signal — the tests are not asserting on the right things. KILL step iterates back here.
...
- MUST NOT decide WHETHER a test exists. That is `tdd`'s Iron Law.
- MUST NOT decide coverage thresholds. That is `tdd`'s 100%-on-business-layer default + documented exceptions.
...
| Need to mock 5 things to test 1 thing | Coupling smell. Inject dependencies via parameters. Compose with `hexagonal-architecture`. |
