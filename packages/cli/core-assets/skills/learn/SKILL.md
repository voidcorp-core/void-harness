...
Do not run it on an unfinished cycle — wait until the work is verified done (`verify`). The merge that closes the cycle is the natural trigger.
...
- **`verify`** — upstream: a cycle is not "closed" until the work is verified.
...
- **`claude-md`** — governs the doc a rule lands in.
- **`decide`** — a structural decision with a rejected alternative is an ADR, not a doctrine line.
