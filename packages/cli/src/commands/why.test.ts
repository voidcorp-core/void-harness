...
it('refuses arity, options and escaping paths without invoking the builder', async () => {
 const build = vi.fn(async () => { throw new Error('must not build'); });
 ...
 } finally { output.mockRestore(); exit.mockRestore(); }
});
it('renders the actionable build cause and exits one when no graph is available', async () => {
 let rendered = '';
 const output = vi.spyOn(process.stdout, 'write').mockImplementation(chunk => { rendered += String(chunk); return true; });
 const exit = vi.spyOn(process, 'exit').mockImplementation(() => { throw new Error('exit'); });
 try {
  await expect(why(['src/a.ts'], async () => { throw new Error('project root disappeared'); })).rejects.toThrow('exit');
