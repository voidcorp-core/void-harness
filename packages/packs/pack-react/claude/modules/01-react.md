...
- Honor accessibility from the first render (composes with `void:accessibility-first`)
...
Minimum 44×44 px (iOS HIG) / 48×48 dp (Material). Use `<Tappable>` from `@repo/ui` which enforces this automatically. Composes with `void:accessibility-first`.
...
- `void-nextjs` — App Router puts components in `apps/<app>/src/components/`.
- `void-mobile` — React Native shares the same primitive contract (Tappable, tokens).
- `void:frontend-design` — anti AI-slop, sober palette, real density.
- `void:accessibility-first` — keyboard, screen reader, color contrast, touch targets.
