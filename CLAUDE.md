...
`autonomous-backlog-loop` has been **removed**. Its out-of-session `claude -p` execution lost the in-session MCP / connector / subscription inheritance, so its capability is being **consolidated into `backlog-autopilot`** — a single in-session skill that drains a Linear pool into **one clean PR per logical cluster** (auto-detecting cluster vs batch-of-4), with an adaptive per-ticket quality cycle and risk-gated auto-merge. Design: `docs/specs/2026-06-21-backlog-autopilot.md`; plan: `plans/2026-06-21-backlog-autopilot-plan.md` (in progress). The durable boundaries carry over unchanged: HITL at backlog curation and PR merge, **commit-only** workers, server-side branch protection required on the base, security hooks (`protect-sensitive-files`, `block-dangerous-bash`) live, `--dangerously-skip-permissions` sandbox-gated, subscription billing (subagents inherit the session auth). A future **headless backend** of `backlog-autopilot` is reserved for walk-away/cron, deferred (not the deleted loop).
