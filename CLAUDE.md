...
`autonomous-backlog-loop` (core skill) drains a curated Linear backlog ticket by ticket, each in a fresh `claude -p` session, with a live append-only flux and a dense final summary. It is **never a default** — it runs only when a human launches `void-harness backlog-loop` (or `/void-backlog-loop` inside Claude Code). The TypeScript orchestrator lives in `packages/cli/src/lib/backlog/`. HITL is preserved at the boundaries: the human curates the backlog (acceptance criteria = the approved spec) and owns the PR merge (no auto-merge unless `--auto-merge` + green CI). The worker is **commit-only**: `git push`/`gh pr` are removed from its allowlist and each iteration runs in its own git worktree; the trusted orchestrator pushes the branch (explicit refspec, no force) and opens the PR, and preflight requires server-side branch protection on the base — that is the durable A1 boundary, `block-protected-push` is only a secondary net. Token usage bills the Claude subscription (API creds stripped from the worker env unless `--allow-api`). The security hooks (`protect-sensitive-files`, `block-dangerous-bash`) stay live and `--dangerously-skip-permissions` is sandbox-gated. The distilled Ralph principles (deterministic backpressure, disposable-but-persistent plan, fresh context per ticket, don't-assume-not-implemented) live in the skill. See `docs/DECISIONS.md` (2026-06-18, 2026-06-19).

`backlog-batch` (core skill) is its **attended, parallel** sister: a human launches `/harness:backlog-batch` to drain several *independent* Linear tickets at once, each in its own **worktree subagent**, reconciled into **one integration PR** gated by the full suite. An in-session launcher selects the independent batch, estimates each ticket's file footprint, and routes **parallel where the overlap risk is low, sequential where it is high** (lockfile/migrations always sequential), then — after **human confirmation** — runs a deterministic **Workflow** for the fan-out. The deterministic core (selection, partition, plan) is the CLI `void-harness backlog-batch plan`; subagents inherit the parent auth → subscription. Needs the Workflow tool. See `docs/DECISIONS.md` (2026-06-18).
