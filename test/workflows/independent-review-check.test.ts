...
function reviewRun(number: number, head: string, run: ReviewRun, over: Record<string, unknown> = {}) {
  return {
    ...
    // On `pull_request_target` GitHub reports the pull request head here.
    head_sha: head,
...
describe('independent review verdict check', () => {
  ...
  it('believes only runs of the review workflow, from the default branch, for this exact head', async () => {
    ...
    // A run's `head_sha` is the pull request head on `pull_request_target`, observed
    // on the first real run: it says nothing about where the workflow came from.
