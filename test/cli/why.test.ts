...
function fixture() { const root = mkdtempSync(join(tmpdir(), 'void-why-cli-')); roots.push(root); return root; }
...
function run(root: string, ...args: string[]) { return spawnSync(process.execPath, [CLI, ...args], { cwd: root, encoding: 'utf8', timeout: 15000, maxBuffer: 1024 * 1024 }); }
...
it('help and invalid argv leave an empty consumer unchanged', () => {
 ...
 for (const args of [[], ['--json'], ['one', 'two'], ['../outside']]) expect(run(root, 'why', ...args).status).toBe(2);
...
it('reads changed and deleted declaration sources despite a persisted knowledge artifact', () => {
 ...
 expect(readFileSync(join(root, '.void/knowledge.json'), 'utf8')).toBe(artifact);
});
