...
# Audit: harness-monorepo:turbo-pipeline-tuning
...
**Composes with.** `harness-monorepo:service-package` (new packages declare their tasks). `harness-monorepo:dependency-direction` (`^build` needs accurate package.json deps). `harness:tdd` (`test` task must have `"outputs": []`).
