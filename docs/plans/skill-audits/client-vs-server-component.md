...
# Audit: harness-react:client-vs-server-component
...
**Composes with.** `harness-nextjs:cache-component-pattern` (Server Components are the cache substrate). `harness-react:state-architecture` (where client state lives once you've decided client). `harness-react:form-pattern` (Server Actions on Server-rendered forms). `harness:hexagonal-architecture` (Server Components ARE the boundary).
