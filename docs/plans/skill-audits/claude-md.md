...
phase: B
depends_on: []
composes_with: [context, source-driven-development, harness-evolution, capture-rule]
matrix_row: plans/skill-decision-matrix.md#claude-md
...
Quote the relevant cells from `plans/skill-decision-matrix.md#claude-md` (to be added in the same change set by the matrix owner; this audit governs the wording, and per the no-shared-file constraint this audit does not edit the matrix itself):
...
- **Composes with**: `context` (context is the constraint), `source-driven-development` (deferred docs ground config), `harness-evolution` (governs harness-generated CLAUDE.md), `capture-rule` (routes a stated rule to its enforcer).
...
- **With `context`**: the parent principle. Context is the agent's core constraint; CLAUDE.md spends from the same budget every task pays. This skill is that principle applied to the always-on prompt. Shared mental model: attention is finite.
...
- **Sequencing**: understand the budget (`context`) → author/audit the file (this skill) → route each rule to its enforcer (`capture-rule` / hooks / settings / deferred doc grounded by `source-driven-development`) → judge harness-wide additions (`harness-evolution`).
...
- MUST NOT decide the project's architecture/invariants or whether to adopt a tool (defers to the project / `brainstorm` / `harness-evolution`).
...
- [ ] Matrix row added at `plans/skill-decision-matrix.md#claude-md` matching this audit (by the matrix owner; not edited here per the shared-file constraint)
- [ ] Skill test in `test/claude-md/` exercises at least 2 fixtures (e.g. a bloated CLAUDE.md to prune, a snippet/style-inlining case to route out)
- [ ] No overlap > 30% with `context` (that skill manages the *session* window; this one manages the *always-on prompt* artifact) or `capture-rule` (that captures a rule; this routes by kind and governs form)
