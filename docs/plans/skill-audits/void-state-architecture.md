...
# Audit: harness-react:state-architecture
...
**Loses to.** Form state (handled by form-pattern). Offline-write state (handled by offline-first-mutation in harness-pwa).

**Composes with.** `harness-react:client-vs-server-component` (server state = server component). `harness-react:form-pattern` (forms = local state via react-hook-form). `harness-pwa:offline-first-mutation` (capture-queue is a specialized client store for sync). `harness-nextjs:cache-component-pattern` (server cache strategy).
