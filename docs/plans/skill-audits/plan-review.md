...
phase: D
depends_on: []
composes_with: [plan, brainstorm, frontend-design, security-guidance, implement]
...
## Decision: one skill with four lenses (not 4-5 skills, not a plan section)
...
- **Not a section in `plan`.** Authoring a plan and adversarially critiquing it from four personas are different subjects; folding the lenses into `plan` would bloat it and create the >30% overlap the ticket explicitly warned against. The boundary is: `plan` authors and owns plan structure/registries; `plan-review` critiques and proposes findings; the author disposes.
