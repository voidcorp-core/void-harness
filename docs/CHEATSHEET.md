...
**Skills advise, agents audit, hooks enforce.** The doctrine holds because the last layer is not optional: 41 core skills, 28 pack skills, 21 agents, 33 hooks.
...
| `review-verdict-write` | Refuses a review verdict status or comment written by hand, or one it cannot read before it runs, so autopilot verdict stays its only writer. |
