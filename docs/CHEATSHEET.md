...
**Skills advise, agents audit, hooks enforce.** The doctrine holds because the last layer is not optional: 37 core skills, 28 pack skills, 21 agents, 31 hooks.
...
| `backlog-autopilot` | Opt-in parallel drain of independent Linear tickets, each in its own worktree subagent, reconciled into one integration PR gated by the full suite. |
...
| | |
|---|---|
| `boundary-direction-check` | Refuses an import that points the wrong way across a package boundary. |
...
The surface no sentence can trigger, because it reports or changes state.

| | |
|---|---|
| `backlog-autopilot` | Attended parallel backlog drain — work several independent Linear tickets at once, each in its own worktree subagent, reconciled into one integration PR. |
...
| `void-graph` | Run the bundled void-graph on the installed harness — no arg opens the local live studio (localhost, offline); audit/cost/behavior print a terminal report of dead/underused/expens… |
