...
tie a ticket to the per-ticket execution cycle (ticket-runner).
...
3. Two harness-tying slots: TDD-mode-per-ticket + "runner passes that apply", so
   ticket-runner consumes a ticket that already declares its conditional passes.
...
multi-ticket pool with native dependencies, `ticket-writer` creates
