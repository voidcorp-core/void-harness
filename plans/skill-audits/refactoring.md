...
- **Loses to**: `tdd` if ANY behavior changes (refactor stops at the boundary of behavior change). `migrations-safety` for DB schema changes (different discipline)
...
| Martin Fowler "Refactoring" 2nd ed. 2018 | https://martinfowler.com/books/refactoring.html | foundation | kept — catalog of named refactors with mechanics + smells catalog |
...
- **Big-bang rewrites**: rejected. "Let's rewrite this module" is not a refactor — it's a feature project. Goes through `brainstorming` + `writing-plans` + `tdd`. Why: rewrites lose context, regress invariants, and never quite catch up to the original.
...
- **Pattern-target refactoring as a default** (Kerievsky): rejected as a daily practice. Reach for it when a specific GoF pattern is the documented destination, not as exploration. Why: patterns are tools, not goals; "refactoring toward Strategy" is often over-design.
- **Speculative refactoring** for future flexibility: rejected. YAGNI. Refactor when current code resists current task. Why: speculative abstractions cost more than they save in 80%+ of cases.
...
- **No big-bang rewrites without an approved plan**. A rewrite project goes through `brainstorming` → `writing-plans` first. Enforced by: SKILL.md + `code-review` blocks PRs with > N changed files touching the same module without a linked plan.
...
`exploratory` mode does NOT apply to refactoring — exploratory code is meant to be thrown away, not refactored.
...
- **With `tdd`**: the R of RED-GREEN-REFACTOR delegates to this skill. `tdd` cycles, `refactoring` decides which move + executes mechanically. The mode of `refactoring` mirrors the mode of `tdd`.
- **With `testing`**: tests must stay green at every step. `testing` decides what makes a good test; `refactoring` must not break the existing ones nor add new ones (new tests = behavior change).
- **With `code-review`**: code-review surfaces refactor candidates ("this function has 4 levels of nesting → Extract Function"). The `refactoring` skill decides if the refactor is worth doing now + executes.
- **With `hexagonal-architecture`** / **`domain-driven-design`**: when a refactor crosses architectural boundaries (Move Class across packages), the architecture skill decides the target placement; `refactoring` does the mechanical move.
...
- [ ] Skill tests in `test/refactoring/` cover: mixed-intent commit warning, unnamed refactor warning, name allowlist check, mode auto-detection from `tdd` companion mode
- [ ] No overlap > 30% with `tdd` (this skill = the R step; tdd = the cycle)
- [ ] No overlap > 30% with `code-review` (review surfaces; refactoring executes)
