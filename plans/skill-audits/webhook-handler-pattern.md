...
# Audit: void-server:webhook-handler-pattern
...
**Composes with.** `void:async-safety` (doctrine). `void-server:server-action` (both cross trust boundary). `void-server:drizzle-migration-safe` (inbox table migration). `void-server:env-validation` (webhook secrets). `void:observability` (trace context). `void:security-guidance` (Zod re-validation).

**Why not in core.** Webhook handling is a server concern with very specific per-source patterns. Stack-agnostic doctrine lives in `void:async-safety`.
