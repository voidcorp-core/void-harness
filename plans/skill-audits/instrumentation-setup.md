...
# Audit: void-nextjs:instrumentation-setup
...
**Loses to.** Non-Next projects (Vite, Remix, Hono). Projects without Sentry / OTel (use `void:observability` doctrine alone).

**Composes with.** `void:observability` (doctrine — this skill is wiring). `void:security-guidance` (Sentry user scope hashed). `void-server:env-validation` (DSN env validated). `void-nextjs:loading-error-boundaries` (error.tsx uses captured Sentry).
