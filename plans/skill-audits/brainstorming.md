...
/spec is a 5-phase intent→spec engine split across brainstorming + writing-plans (DEV-388). **Integrated here** (the precision/grounding half): read-code-before-asking (cite `path:line`, "don't ask what you can read" — the strongest delta; this skill asked questions but never mandated evidence-grounding), the five mandatory "why" questions as a gate (esp. *verified* current behavior + the observable done-signal), quantify-everything (exact counts, metric+target), and failure-mode thinking as a first-class question axis (empty/null/enormous/dup/wrong-role/twice). **Deliberately NOT integrated**: /spec's single-solution "relentless" persona was not allowed to overwrite this skill's divergent 2-3-approaches step (spec converges, brainstorming explores — kept both). **Rejected**: /spec's plan-mode runtime + Phase-5 file/dispatch machinery. The executability gate + MVP-cut half went to writing-plans.
