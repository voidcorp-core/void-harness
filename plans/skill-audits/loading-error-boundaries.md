...
# Audit: void-nextjs:loading-error-boundaries
...
**Composes with.** `void-nextjs:cache-component-pattern` (cached pages mostly skip loading.tsx). `void-nextjs:parallel-routes-slots` (per-slot boundaries). `void:observability` (error.tsx is the Sentry capture point). `void-react:accessibility-check` (skeletons need aria-busy / aria-live).
