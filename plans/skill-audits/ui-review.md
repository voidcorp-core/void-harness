...
## Decision: frontend-design (build) + ui-review (audit), forge for recon, browser deferred
...
- **impeccable internalised, not depended on.** Its prose methodology is vendored (audit/critique/refine); its browser runtime (scripts, reference command files, `live`, comparison board) is deferred to Vague 4.
...
| impeccable | scripts/, reference/*.md command files, `live`, comparison board, pin/hooks | **defer Vague 4** | browser driver / runtime |
...
| design-review | live screenshots, atomic-fix loop, cross-page click flows | **defer Vague 4** | browser driver |
...
- Deferred to Vague 4 (claude-in-chrome MCP): every live-browser piece — screenshots, `live`/variant iteration, comparison board, atomic-fix loop. The skill marks this boundary explicitly.
...
Anti-bloat (86 LOC ≤ 400, desc ≤ 200, name==folder, `.source` + this note), core-assets mirror, graph regen, full test suite, CLAUDE.md↔AGENTS.md parity. Behavioral eval deferred to DEV-397 (a UI critique is a conversational/visual artifact the v1 eval harness cannot score).
