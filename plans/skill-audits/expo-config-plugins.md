...
# Audit: harness-mobile:expo-config-plugins
...
**Composes with.** `harness-mobile:eas-build-profile` (plugins define what's IN, profiles define HOW to build). `harness-mobile:ota-update-strategy` (plugin changes break OTA — runtime fingerprint shift). `harness-react:state-architecture` (JS state shared with web). `harness-server:env-validation` (env via `extra` validated).
