...
- **Cannot decide**: tactical patterns delegated to `functional` + `hexagonal-architecture`. Sub-domain analysis (Core / Supporting / Generic) — that is a product call, lives upstream in `office-hours` / `plan-ceo-review`
...
- **Sub-domain analysis upstream**: identifying Core / Supporting / Generic sub-domains is a product call (which capabilities are competitive advantage vs commodity). We delegate this to `office-hours` and `plan-ceo-review` (gstack). The DDD skill consumes the result, does not produce it.
